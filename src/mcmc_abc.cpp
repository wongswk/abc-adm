...
////////////////////////////////////////////////////////////////////////////////
	...
	// read data
	const double k = 388440; // MOR PSI per hour
	...
	vector<double> w0(n_file); // MOR PSI
	vector<double> censored(n_file); // 1 year in hours
	vector<double> T0(n_file); // hour

	string length;
	int pos1, pos2, pos3;
	vector< vector<double> > load(n_file), constdata(n_file);
	...
	for(int i = 0; i < n_file; ++i){
		...
		string::size_type pos3 = file_name[i].find('.');

		// extract w0, T0, censored form the file names
		w0[i] = stod(file_name[i].substr(pos1+1, pos2-pos1-1));
		T0[i] = w0[i]/k;
		...
		if(length.back() == 'Y'){
			...
			if(length.back() == 'M'){
				censored[i] = stod(length.substr(0, length.size()-1)) * 720;
			}
			else{
		...
		// read data
		{
			...
			if(fin.is_open()){
				...
				while(getline(fin, line)){
						...
						constdata[i].push_back(convertToDouble(line.substr(pos+1)));
				}
			}
			else{
